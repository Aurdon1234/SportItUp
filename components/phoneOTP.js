...
export default function PhoneOTP({ initialPhone = "", onVerified }) {
  ...
  const verifyOtp = async () => {
    setError("");
    if (!sessionId) { setError("No OTP session"); return; }
    setLoading(true);
    try {
  ...
  const resend = async () => { 
    if (resendTimer > 0) return;
    setOtp(""); setSessionId(null); setOtpSent(false);
    await sendOtp();
  };
  ...
  return (
    <div className="space-y-2">
      <label className="block text-sm font-medium text-gray-700">Phone</label>
      <div className="flex gap-2">
        <input value={phone} onChange={(e)=>setPhone(e.target.value)} placeholder="+91 99999 99999" className="border rounded px-3 py-2 flex-1" />
        {!otpSent ? (
          ...
          <>
            <input value={otp} onChange={(e)=>setOtp(e.target.value)} placeholder="OTP" className="border rounded px-3 py-2 w-28" />
            <button onClick={verifyOtp} disabled={loading} className="bg-blue-600 text-white px-3 py-2 rounded">{loading ? "Verifying..." : "Verify"}</button>
