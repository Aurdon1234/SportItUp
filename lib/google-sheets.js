...
import { TURF_SHEET_MAP } from "@/lib/turf-sheets";
...
// export async function getSheetsClient(turfID) {
...
export async function getSheetsClient(turfId) {
  ...
  assert(clientEmail, "Missing Google service account email");
  assert(rawPrivateKey, "Missing Google private key");

  const config = TURF_SHEET_MAP[turfId];
  if (!config) {
    throw new Error(`No Google Sheet configured for turfId: ${turfId}`);
  }

  const { spreadsheetId, sheetTitle } = config;
...
export async function appendBookingRow(rowObj = {}) {
  ...
  const { turfId } = rowObj;
  assert(turfId, "turfId is required in rowObj to append booking row");
 ...
 */
export async function getAllBookingRows(turfId) {
  const client = await getSheetsClient(turfId);
  const { sheets, spreadsheetId, sheetTitle } = client;
...
export async function getBlockedSlotsFor(turfIdOrName, date, court) {
  ...
  const rows = await getAllBookingRows(turfId);
