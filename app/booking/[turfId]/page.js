...
//   /**
//    * Step 2: UI — disable/grey out booked slots and prevent selection.
//    * toggleTimeSlot will refuse to toggle a blocked slot.
//    */
//   const toggleTimeSlot = (time) => {
//     if (blockedHours.has(time)) return; // blocked -> no action
//     setSelectedTimeSlots((prev) => {
//       const next = new Set(prev);
//       if (next.has(time)) next.delete(time);
//       else next.add(time);
...
//   /**
//    * Step 3 (continued): Before proceeding to payment, re-check availability once more
//    * to avoid race conditions (someone else may have just booked the same slots).
//    */
//   const handleProceedToPayment = async () => {
//     const needsSportSelection = turf.sports.length > 1 && !selectedSport;
//     if (slotsCount === 0 || !selectedDate || needsSportSelection) {
...
//         if (conflicts.length > 0) {
//           alert(`Sorry — these slots were just booked: ${conflicts.join(", ")}. Please reselect.`);
//           // revalidate UI to reflect the new state
//           mutate(availabilityUrl);
//           return;
//         }
//       } catch (err) {
//         console.error("Failed to refresh availability:", err);
//         // We choose to continue in fail-open mode here, but you may prefer to block.
//       }
//     }
...
export default async function Page({ params, searchParams }) {
  const turfId = params.turfId;
  // use search param `date` if present, else today
  const date = searchParams?.date || new Date().toISOString().split("T")[0];

  // Build absolute URL for server-side fetch; set NEXT_PUBLIC_SITE_URL in env for production
  ...
  try {
    ...
    if (res.ok) {
      ...
      initialBlockedHours = json?.blockedHours || [];
    } else {
