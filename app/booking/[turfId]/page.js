...
export default function BookingPage() {
  ...
  const blockedHours = avail?.blockedHours || [];

  const toggleTimeSlot = (time) => {
    if (blockedHours.includes(time)) return;
    setSelectedTimeSlots((prev) => {
      const next = new Set(prev);
      if (next.has(time)) next.delete(time);
      else next.add(time);
  ...
  const handleProceedToPayment = () => {
    ...
    const bookingData = {
      ...
      advanceAmount: Math.round(getCurrentPrice() * 0.1),
      remainingAmount: getCurrentPrice() - Math.round(getCurrentPrice() * 0.1),
    };
    const queryParams = new URLSearchParams({ booking: JSON.stringify(bookingData) }).toString();
    window.location.href = `/payment?${queryParams}`;
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 py-8">
        <Card className="border-gray-200">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2 text-black">
              <Clock className="w-5 h-5 text-green-600" /> Select Time Slot
            </CardTitle>
            <CardDescription>
              {selectedDate ? `Slots for ${selectedDate.toDateString()}` : "Please select a date"}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {timeSlots.map((slot) => {
                const isBlocked = blockedHours.includes(slot.time);
                const isSelected = selectedTimeSlots.has(slot.time);
                return (
                  <Button
                    key={slot.time}
                    ...
                    onClick={() => !isBlocked && toggleTimeSlot(slot.time)}
                    disabled={isBlocked}
                  >
                    <span className="font-medium">{slot.label}</span>
                    <span className="text-xs text-gray-600">₹{getCurrentPrice()}</span>
                    {isBlocked && (
                      <span className="absolute bottom-1 text-[10px] text-gray-600">Booked</span>
                    )}
                  </Button>
                );
              })}
